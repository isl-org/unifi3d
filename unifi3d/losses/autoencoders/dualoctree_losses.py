...
def sdf_reg_loss(sdf, grad, sdf_gt, grad_gt, name_suffix=""):
    wg, ws = 1.0, 200.0
    assert not torch.isnan(grad).any(), "NaN detected in gradients!"
    assert not torch.isnan(grad_gt).any(), "NaN detected in gt grad!"

    grad_loss = (grad - grad_gt).pow(
        2
    ).mean() * wg  # When running through all items, this gives an error that is out of scope.
...
def shapenet_loss(batch, model_out, reg_loss_type="", update_loss=True):
    # octree loss
    ...
    if update_loss:
        for key in model_out:
            if "loss" in key:
                print(f"Updated {key} loss {model_out[key]}")
                output.update({key: model_out[key]})
    return output


class DualOctreeShapeNetLoss(nn.Module):
    def __init__(
        self,
        ...
        self.extra_loss_start = extra_loss_start
        self.extra_loss_end = 10 ** (extra_loss_end_exponential)
        self.extra_loss_anneal_rate = 10**extra_loss_anneal_rate_exponential
        self.enable_anneal = enable_anneal
        self.start_epoch = start_epoch

    def update_extra_loss_weight(self, epoch):
        self.extra_loss_weight = min(
            self.extra_loss_end,
            self.extra_loss_start
            + self.extra_loss_anneal_rate
            * ((self.start_epoch + epoch) * (epoch - self.start_epoch + 1) / 2),
        )

    def forward(self, data, model_out):
        output = shapenet_loss(
            batch=data,
            model_out=model_out,
        ...
        for key in model_out:
            if "loss" in key:
                print(f"Updated {key} loss.")
                loss = model_out[key]
                if key == "loss_kl" or key == "loss_quantize":
                    loss_name = key.replace("loss", "weight")
                    if self.enable_anneal:
                        loss = loss * self.extra_loss_weight
                        output.update({loss_name: self.extra_loss_weight})
                    else:
                        output.update({loss_name: 1.0})
                output.update({key: loss})
        return output
