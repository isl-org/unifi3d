...
from unifi3d.models.modules.diffusion.ldm_diffusion_util import (
    checkpoint,
    conv_nd,
    linear,
    avg_pool_nd,
...
from unifi3d.models.modules.diffusion.attention import (
    SpatialTransformer,
...
class Upsample(nn.Module):
    ...
    :param use_3d_aware_conv: if True, use 3D aware convolutions for triplane representation
    """
    ...
    def __init__(
        ...
        self.dims = dims
        if use_conv:
            if dims == 2 and use_3d_aware_conv:
                self.conv = GroupConv(  # Conv3DAware(
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        if self.out_channels == channels:
            ...
            if dims == 2 and use_3d_aware_conv:
                self.skip_connection = GroupConv(  # Conv3DAware(
                    in_channels=channels,
                    out_channels=self.out_channels,
                    kernel_size=3,
                    stride=1,
...
class AttentionBlock(nn.Module):
    ...
    def _forward(self, x):
        ...
        # TODO: temporary fix for this line, this line breaks occasionally if the input is not float for some reason.
        qkv = self.qkv(normed_x.float()).to(x)
        h = self.attention(qkv)
        # TODO: temporary fix for this line, this line breaks occasionally if the input is not float for some reason.
        h = self.proj_out(h.float()).to(x)
...
class UNet3DModel(nn.Module):
    ...
    :param attention_resolutions: a collection of downsample rates at which
        ...
        will be used.
    :param conditioning_modes: dictionary of mode and type of conditioning, e.g.
        {"text": "concat"}
    ...
    :param use_3d_aware_conv: use 3D aware convolution layers for triplanes (requires dim = 2)
    """
    ...
    def __init__(
        ...
        **conditioning_modes,
        ...
        # prepare encoders for conditioning
        if "category" in conditioning_modes:
            assert num_classes is not None
            self.y_embedder = nn.Embedding(num_classes, context_dim)
        if "text" in conditioning_modes:
            self.text_encoder = ClipTextEncoder(context_dim)
        if "image" in conditioning_modes:
            self.img_encoder = ClipImageEncoder(context_dim)
            ...
            for _ in range(num_res_blocks):
                layers = [
                    ResBlock(
                        ...
                        use_3d_aware_conv=use_3d_aware_conv,
                    )
                ]
    ...
    def forward(self, x, timesteps, context_in={}, **kwargs):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of noisy inputs.
        :param timesteps: a 1-D batch of timesteps.
        :param context_in: conditioning input dict, e.g. {"text": ["airplane", "car"]}
        :return: an [N x C x ...] Tensor of outputs.
        """
        # encode all the context
        context_encoded = {}
        if "text" in self.conditioning_modes:
            context_encoded["text"] = self.text_encoder(context_in["text"]).cuda()
        if "category" in self.conditioning_modes:
            context_encoded["category"] = self.y_embedder(context_in["category"])
        if "image" in self.conditioning_modes:
            context_encoded["image"] = self.img_encoder(context_in["image"])
        ...
        context = None
        # Determine the conditioning method and apply the diffusion model accordingly
        for cond_mode, cond_type in self.conditioning_modes.items():
            context_to_add = context_encoded[cond_mode]
            if cond_type == "concat":
                assert (
                    x.size()[2:] == context_to_add.size()[2:]
                ), "For concat, shapes of x and context must match apart from dim 1"
                x = torch.cat([x, context_to_add], dim=1)
            elif cond_type == "crossattn":
                if context_to_add.dim() == 2:
                    # if the context is 1-dim (+bs), we add 1 dim to concat with others
                    context_to_add = context_to_add.unsqueeze(1)
                # if already something in the context, add it, else initialize
                if context is None:
                    context = context_to_add
                else:
                    context = torch.cat([context, context_to_add], 1)
            else:
                # Raise an error if an unsupported conditioning method is used
                raise NotImplementedError()
        if context is not None:
            assert (
                self.use_spatial_transformer is not None
            ), "if using context with crossattn, must use spatial transformer"

        assert self.num_classes is None or (
            "category" in self.conditioning_modes
        ...
        if self.predict_codebook_ids:
            ...
            if self.reshape_input:
                h = self.out(h)
                return self.x_embedder.unpatchify(h)
            else:
                return self.out(h)
