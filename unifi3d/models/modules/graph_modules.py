...
from unifi3d.utils.model.scatter_utils import scatter_mean
...
class GraphConv(torch.nn.Module):
    def __init__(
        self, in_channels, out_channels, n_edge_type=7, avg_degree=7, n_node_type=0
    ):
        super().__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
    ...
    def extra_repr(self) -> str:
        return (
            "channel_in={}, channel_out={}, n_edge_type={}, avg_degree={}, "
            "n_node_type={}".format(
                self.in_channels,
                self.out_channels,
                self.n_edge_type,
...
class Upsample(torch.nn.Module):
    def __init__(self, channels):
        ...
        self.weights = torch.nn.Parameter(torch.Tensor(channels, channels, 8))
...
class GraphDownsample(torch.nn.Module):
    ...
    def forward(self, x, leaf_mask, numd, lnumd):
        ...
        out = torch.zeros(
            leaf_mask.shape[0], x.shape[1], device=x.device, dtype=x.dtype
        )
