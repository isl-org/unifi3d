...
class DualOctree:
    ...
    def _lookup_table(self):
        ...
        self.remap = torch.tensor(
            [1, 0, 3, 2, 5, 4], dtype=torch.int64, device=self.device
        )
        ...
        self.inter_dir = torch.tensor(
            [0, 2, 4, 1, 2, 4, 3, 0, 4, 3, 1, 4, 5, 0, 2, 5, 1, 2, 5, 3, 0, 5, 3, 1],
            dtype=torch.int64,
            device=self.device,
        )
    ...
    def _node_depth(self):
        nd = [
            torch.tensor([d], dtype=torch.int64, device=self.device).expand(
                self.nnum[d]
            )
    ...
    def dense_graph(self, depth=3):
        ...
        bnd = 2**depth  # number of subdivisions along each axis.
        num = bnd**3  # total number of smaller octants
        ...
        return {
            "edge_idx": torch.stack([row.view(-1), col.view(-1)]),
    ...
    def add_self_loops(self):
        for d in range(self.full_depth, self.depth + 1):
            ...
            self.graph[d] = {
                "edge_idx": torch.stack(
                    [torch.cat([row, unique_idx]), torch.cat([col, unique_idx])]
                ),
    ...
    def post_processing_for_docnn(self):
        # Map octree to doctree edge_idx and edge_dirs.
        self.add_self_loops()
        self.remap_node_idx()
        self.add_node_type()
