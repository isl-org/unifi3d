...
# from external.ldm.modules.diffusionmodules.util import make_ddim_sampling_parameters, make_ddim_timesteps, noise_like
    ...
    def p_sample_plms(
        ...
        def get_model_output(x, t):
            if (
                ...
                e_t = self.model.apply_diff_model(x, t, c)
            else:
                x_in = torch.cat([x] * 2)
                t_in = torch.cat([t] * 2)
                c_in = torch.cat([unconditional_conditioning, c])
                e_t_uncond, e_t = self.model.apply_diff_model(x_in, t_in, c_in).chunk(2)
