...
from unifi3d.utils.triplane_utils.layers import ResnetBlockFC
from torch_scatter import scatter_mean, scatter_max
from unifi3d.utils.triplane_utils.common import (
    coordinate2index,
    normalize_coordinate,
    normalize_3d_coordinate,
    map2local,
)
from unifi3d.utils.triplane_utils.encoder.unet import UNet
from unifi3d.utils.triplane_utils.encoder.unet3d import UNet3D
...
class LocalPoolPointnet(nn.Module):
    ...
    def generate_plane_features(self, p, c, plane="xz"):
        ...
        )  # sparce matrix (B x c_dim x reso x reso)
        ...
        if self.unet is not None:
            ...
            if self.use_layer_norm:
                fea_plane = self.layer_norm(fea_plane.permute(0, 2, 3, 1)).permute(
                    0, 3, 1, 2
                )
        return fea_plane
    ...
    def generate_grid_features(self, p, c):
        ...
        if self.unet3d is not None:
            ...
            if self.use_layer_norm:
                fea_grid = self.layer_norm(fea_grid.permute(0, 2, 3, 4, 1)).permute(
                    0, 4, 1, 2, 3
                )
