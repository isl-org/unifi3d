...
class LocalDecoder(nn.Module):
    ...
    def get_encoded_latent(self, p, c_plane):
        if self.c_dim != 0:
    ...
    def forward(self, p, c_plane, **kwargs):
        if isinstance(c_plane, dict):
            c = self.get_encoded_latent(p, c_plane)
        elif isinstance(c_plane, torch.Tensor):
            c = c_plane
        else:
            raise RuntimeError("c_plane should be a dict or tensor")
        p = p.float()
        net = self.fc_p(p)
...
class PatchLocalDecoder(nn.Module):
    ...
    def get_encoded_latent(self, p_n, c_plane):
        if self.c_dim != 0:
            plane_type = list(c_plane.keys())
            c = 0
            if "grid" in plane_type:
    ...
    def forward(self, p, c_plane, **kwargs):
        ...
        p = p["p"]
        if isinstance(c_plane, dict):
            c = self.get_encoded_latent(p_n, c_plane)
        elif isinstance(c_plane, torch.Tensor):
            c = c_plane
        else:
            raise RuntimeError("c_plane should be a dict or tensor")
...
class LocalPointDecoder(nn.Module):
    ...
    def get_encoded_latent(self, p, c):
        n_points = p.shape[1]
