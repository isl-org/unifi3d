...
class Shapes3dDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for field_name, field in self.fields.items():
            # try:
            field_data = field.load(model_path, idx, info, self.data_saved)
            # except Exception:
            #     if self.no_except:
            #         logger.warn(
        ...
        #     save_np_pointcloud(data["inputs"], path, f"pcl{self.data_iter}")
        #     occ, unocc = get_points_from_occupancy(data["points.occ"], data["points"])
        #     print(f"Occ: {occ.shape}, Unocc: {unocc.shape}")
